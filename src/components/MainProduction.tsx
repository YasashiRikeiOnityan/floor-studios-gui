...
type MainProductionProps = {
  type: string | undefined;
  callBackUpdateState: () => void;
};

const MainProduction = (props: MainProductionProps) => {
  const [quantity, setQuantity] = useState<{ [key: string]: number }>(specificationStore.currentSpecification.mainProduction?.quantity || {});
  const [deliveryDateEnabled, setDeliveryDateEnabled] = useState<boolean>(specificationStore.currentSpecification.mainProduction?.deliveryDate ? true : false);
  const [deliveryDate, setDeliveryDate] = useState<string>(specificationStore.currentSpecification.mainProduction?.deliveryDate || "");
  ...
  const handleCancel = () => {
    setQuantity(specificationStore.currentSpecification.mainProduction?.quantity || {});
    setDeliveryDate(specificationStore.currentSpecification.mainProduction?.deliveryDate || "");
    setDeliveryDateEnabled(specificationStore.currentSpecification.mainProduction?.deliveryDate ? true : false);
  }

  const handleSaveAndNext = () => {
    if (props.type === "T-SHIRT") {
      ...
      specificationStore.currentSpecification.mainProduction = {
        quantity: {
          xxs: quantity.xxs,
          xs: quantity.xs,
          s: quantity.s,
        ...
        <div className="space-y-2 max-w-lg">
          ...
          <div className="grid grid-cols-7 gap-2 items-center">
            <input
              ...
              className="block rounded-md bg-white py-1.5 text-center text-base text-gray-900 outline-1 -outline-offset-1 border-gray-300 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"
            />
            <input
              type="text"
              placeholder="0"
