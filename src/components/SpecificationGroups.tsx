...
const SpecificationGroups = observer((props: SpecificationGroup) => {
  ...
  return (
    <Listbox value={props.currentSpecificationGroupId} onChange={props.setCurrentSpecificationGroupId} as="div" className="relative">
      <ListboxButton className={`grid ${props.fullWidth ? "w-full" : "w-80"} cursor-default grid-cols-1 rounded-md bg-gray-50 py-1.5 pl-3 pr-2 text-left text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus:outline-2 focus:-outline-offset-2 focus:outline-blue-600 sm:text-sm/6`}>
